...
uint256 hashGenesisBlock("0x5ca17b85ce10702fd5ce8f63a5657905ae8ba41c7b7a886e8d13b317386dda06");
...
const string strMessageMagic = "MFCoin Signed Message:\n";
...
{
    // MFCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // MFCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mfc genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501719878;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1827771;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x3bfdb5a210d0e61b567f3ffb34cbc70c1567d517eb64e6bbff255fee39d1c4b5"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MFCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MFCoinMiner(CWallet *pwallet)
{
    printf("MFCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MFCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MFCoinMiner, pwallet));
}
