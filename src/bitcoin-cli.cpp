...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") || IsArgSet("-version")) {
        ...
        if (!IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  mfcoin-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  mfcoin-cli [options] -named <command> [name=value] ... " + strprintf(_("Send command to %s (with named arguments)"), _(PACKAGE_NAME)) + "\n" +
                  "  mfcoin-cli [options] help                " + _("List commands") + "\n" +
                  "  mfcoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const std::string& strMethod, const UniValue& params)
{
    ...
    // mfcoin:
    // we get mfcoind output (possibly binary) of .write() that either:
