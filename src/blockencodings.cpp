...
CBlockHeaderAndShortTxIDs::CBlockHeaderAndShortTxIDs(const CBlock& block, bool fUseWTXID) :
    ...
    // mfcoin: get header with correct nFlags and auxpow
    BlockMap::iterator mi = mapBlockIndex.find(block.GetHash());
    assert(mi != mapBlockIndex.end()); // mfcoin: we should always have index for a block that we have accepted
    header = (*mi).second->GetBlockHeader();
    vchBlockSig = block.vchBlockSig;
    header.mtpHashData = block.mtpHashData;
