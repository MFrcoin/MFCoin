...
const QString BITCOIN_IPC_PREFIX("mfcoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/mfcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/mfcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/mfcoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
			...
			const QUrl url = arg;
			if(RandPayRequest::isRandPayUrl(url)) {
				//just skip warning
				continue;
			}
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start mfcoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
		...
		const QUrl url = s2;
		if(RandPayRequest::isRandPayUrl(url)) {
			RandPayRequest::process(url, this);
			return;
		}

		const QUrlQuery uri(url);
		...
		if (GUIUtil::parseBitcoinURI(s, recipients))
		{
			...
			Q_EMIT message(tr("URI handling"),
				tr("URI cannot be parsed! This can be caused by an invalid MFCoin address or malformed URI parameters."),
				CClientUIInterface::MSG_WARNING);
