...
enum
{
    ...
    // modifiers
    SER_POSMARKER       = (1 << 18),  // mfcoin: for sending block headers with PoS marker, to allow headers-first syncronization
    SER_BTC_TX          = (1 << 19),  // mfcoin: for merged mining, to read tx without nTime.
};
...
template<typename Stream, typename ItemType, int ArraySize, typename = CArithType<ItemType>>
inline unsigned int GetSerializeSize(const ItemType (&a)[ArraySize]) { return sizeof(ItemType[ArraySize]); }
...
template<typename Stream, typename ItemType, int ArraySize1, int ArraySize2, typename = CArithType<ItemType>>
inline unsigned int GetSerializeSize(const ItemType (&a)[ArraySize1][ArraySize2]) { return sizeof(ItemType[ArraySize1][ArraySize2]); }
