...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, int nVersion, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    // mfcoin: for backward compatability namecoin script should not be checked for minimaldata
    ...
    // mfcoin: check names in P2SH only after V7 fork
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && fIsP2SH)
    {
        ...
        if (flags & SCRIPT_VERIFY_WITNESS) {
            if (pubKey2.IsWitnessProgram(witnessversion, witnessprogram, 0)) {  // mfcoin: this is a witness program inside scriptSig - we don't store names in scriptSig
                hadWitness = true;
...
size_t CountWitnessSigOps(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, int nVersion)
{
    ...
    if (scriptPubKey.IsPayToScriptHash(nVersion) && scriptSig.IsPushOnly()) {
        ...
        if (subscript.IsWitnessProgram(witnessversion, witnessprogram, 0)) { // mfcoin: this is a witness program inside scriptSig - we don't store names in scriptSig
            return WitnessSigOps(witnessversion, witnessprogram, witness ? *witness : witnessEmpty, flags);
        }
    }
