...
// Copyright (c) 2011-2012 MFCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MFCoinTEST0\r");
            Send(hSocket, "WHO #MFCoinTEST0\r");
        } else {
            // randomly join #MFCoin00-#MFCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MFCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MFCoin%02d\r", channel_number).c_str());
        }
