...
namespace {
    ...
    /** mfcoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // mfcoin: set deserialization mode to read PoS flag in headers
	...
	if(pfrom->temperature > temp_limit && pfrom->temperature - temp_limit > GetRand(temp_limit))
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, ptx, &fMissingInputs, &lRemovedTxn)) {
            // mfcoin: raise temerature for this peer
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            // mfcoin: quick check to see if we should ban peers for PoS spam
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
...

        {
            ...
            // mfcoin: store in memory until we can connect it to some chain
        ...
        bool fContinue = true;
        // mfcoin: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // mfcoin: try to select next block in a constant time
