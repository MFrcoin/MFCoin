...
const char * const BITCOIN_CONF_FILENAME = "mfcoin.conf";
const char * const BITCOIN_PID_FILENAME = "mfcoind.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MFC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MFC
    // Mac: ~/Library/Application Support/MFC
    // Unix: ~/.MFC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MFC";
#else
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MFC";
#else
    // Unix
    return pathRet / ".MFC";
#endif
...
boost::filesystem::path GetDefaultEnvPath()
{
    namespace fs = boost::filesystem;
    // Mac: MFCoin-Qt.app/Contents
    // Windows & Unix: ..
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return fs::path("/Applications/MFCoin-Qt.app/Contents");
#else
    // Unix
    return fs::current_path() / "..";
#endif
...
{
    return "Copyright (С) MFCoin, Emercoin, Bitcoin, PPCoin, Namecoin, Unobtanium Developers";
}
