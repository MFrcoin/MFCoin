...
void Shutdown()
{
    ...
    RenameThread("mfcoin-shutoff");
    mempool.AddTransactionsUpdated(1);
#ifdef ENABLE_WALLET
    GenerateMFCoins(false, 0, Params());
#endif
    StopHTTPRPC();
    StopREST();
    StopRPC();
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/MFCoin/mfcoin>";
    const std::string URL_WEBSITE = "<https://mfcoin.net>";
...
//            return InitError(_("Prune mode is incompatible with -txindex."));
        return InitError(_("Prune mode is disabled in mfcoin."));
    }
...
bool AppInitSanityChecks()
{
    ...
    // mfcoin: init hash seed
    mfcoinRandseed = GetRand(1 << 30);

    // mfcoin: moved here because ECC need to be initialized to execute this
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // mfcoin: allow user to set checkpoint pubkey
    ...
    bool fLoaded = false;
    int fAuxReindex = 0;   // mfcoin: used when upgrading pre-auxpow blockindex
    while (!fLoaded) {
        ...
        if (!fLoaded) {
            ...
            if (!fReset) {
                // mfcoin: try to reindex if no auxpow flag
                bool fAuxPow;
    ...
    // mfcoin: check if indexes need to be created or recreated
    ...
    {
        // mfcoin: remove secondary index if we are re-creating first one (both will be re-created)
    ...
    // mfcoin: recreate secondary (address -> name) index if it was deleted
    ...
    GenerateMFCoins(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
#endif

    // init mfcdns. WARNING: this should be done after hooks initialization
    if (GetBoolArg("-mfcdns", false))
    {
        int port = GetArg("-mfcdnsport", MFCDNS_PORT);
        int verbose = GetArg("-mfcdnsverbose", 1);
        if (port <= 0)
            port = MFCDNS_PORT;
        string suffix  = GetArg("-mfcdnssuffix", "");
        string bind_ip = GetArg("-mfcdnsbindip", "");
        string allowed = GetArg("-mfcdnsallowed", "");
        string localcf = GetArg("-mfcdnslocalcf", "");
        string enums   = GetArg("-enumtrust", "");
        string tf      = GetArg("-enumtollfree", "");
	uint32_t dapzs = GetArg("-dapsize", 0);
	uint32_t dapth = GetArg("-daptreshold", MFCDNS_DAPTRESHOLD);
        mfcdns = new MfcDns(bind_ip.c_str(), port,
