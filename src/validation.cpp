...
#if defined(NDEBUG)
# error "MFCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "MFCoin Signed Message:\n";
CHooks* hooks = InitHook(); //this adds namecoin hooks which allow splicing of code inside standart mfcoin functions.
...
std::vector<boost::filesystem::path> GetUTXOFiles()
{
    std::string default_dir = (GetDataDir() / "utxo_snapshot").string();
    std::string strUtxoPath = GetArg("-utxo-path", default_dir);
    boost::filesystem::path utxo_path(strUtxoPath);
    if (utxo_path.empty() || !utxo_path.has_filename())
    {
       LogPrintf("GetUTXOFiles(): UTXO path is not specified, add utxo-path=<path-to-utxop-files> to your mfcoin.conf and restart");
       return {};
    }

    std::vector<boost::filesystem::path> utxo_files;
    std::string ext(".bin");
    for(auto& p : boost::filesystem::directory_iterator(utxo_path))
    {
        if (p.path().extension() == ext)
           utxo_files.push_back(p.path());
    }
    std::sort(utxo_files.begin(), utxo_files.end());
    return utxo_files;
}

std::string GetUTXOFileName(int nHeight)
{
   std::string forkUtxoPath = GetArg("-utxo-path", (GetDefaultEnvPath() / "utxo_snapshot").string());
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    // mfcoin: only one randpay utxo is allowed per transaction
    bool found = false;
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx,
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fRandPayCheck)
{
    ...
    // mfcoin: rather not accept more than one name op on the same name
...

    {
        ...
        // mfcoin: MIN_TXOUT_AMOUNT was added because of weird behavior of CreateTransactionInner that can add sub-subcents on top of absurd fee.
        ...
        if (fRandPayCheck) // we dont add transaction to mempool if we are just doing randpay check
            return true;

        // Remove conflicting transactions from the mempool
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting)
        {
            LogPrint("mempool", "replacing tx %s with %s for %s MFC additional fees, %d delta bytes\n",
...
bool AcceptToMemoryPoolWithTime(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool fRandPayCheck)
{
    std::vector<uint256> vHashTxToUncache;
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, vHashTxToUncache, fRandPayCheck);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        ...
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool fRandPayCheck)
{
    return AcceptToMemoryPoolWithTime(pool, state, tx, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, fRandPayCheck);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // MTP
    if (block.IsProofOfWork() && !CheckMerkleTreeProof(block, consensusParams)){
...
CAmount GetProofOfWorkReward(unsigned int nBits, bool fV7Enabled, int nHeight)
{
    int nReductions = nHeight / POW_HALVING_PERIOD;
    CAmount reward = POW_REWARD / int64_t(1ull << std::min(nReductions,63));
    return std::max(reward, POW_MIN_REWARD);
}
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // mfcoin: we are using arguments that will have no effects in updateAlert()
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            // mfcoin: skip special randpay utxo, because we don't record undo information for it
            if (txin.prevout.hash == randpaytx)
                continue;
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, bool fV7Enabled)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                // mfcoin: at some point we changed coinbase maturity from 12 to 32
    ...
    if (tx.IsCoinStake())
    {
        ...
        CAmount nLimit = 0;
        if (!GetMfc7POSReward(tx, inputs, nLimit))
    ...
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // mfcoin: skip randpay check to do it as a last step
            ...
            // mfcoin: copy of above checks for randpay tx
...
bool ppcoinContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
  ...
  // compute nStakeModifierChecksum end
    ...
    if (!CheckStakeModifierCheckpoints(pindex->nHeight, nStakeModifierChecksum))
        return error("ConnectBlock() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016llx, checksum=0x%08llx", pindex->nHeight, nStakeModifier, nStakeModifierChecksum);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, bool fWriteNames)
{
    ...
    // mfcoin: check PoS and compute stake modifier if we did not do that earlier
    ...
    if (pindex->pprev && !isForkBlock(pindex->pprev->nHeight+1) && !CheckMinTxOut(block, fV7Enabled))
    ...
    // mfcoin: variables for name fees check, nMoneySupply calculation and trustHeight option
    CAmount nValueIn = 0;
    CAmount nValueOut = 0;
    ...
    // mfcoin: add names to nameindex.dat
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    //moved the check to contextual checker
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    {
        // this is needed only for mfcoin official blockchain, because of mistake we made at the beginning
...
// mfcoin: note, this function can process blocks without strict order
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool fCheckSign)
{
    ...
    if (!isForkBlock(nHeight)) {  //when block is in forking region - don't check coinbase amount
        for (unsigned int i = 1; i < block.vtx.size(); i++)
            if (block.vtx[i]->IsCoinBase())
                return state.DoS(100, false, REJECT_INVALID, "bad-cb-multiple", false, "more than one coinbase");
        // Check coinbase reward
        CAmount powLimit = block.IsProofOfWork() ? GetProofOfWorkReward(block.nBits, fV7Enabled, nHeight) -
                                                  block.vtx[0]->GetMinFee() + MIN_TX_FEE : 0;
    ...
    if (!isForkBlock(nHeight)) {
        for (const auto& tx : block.vtx) {
            // aest: check transaction timestamp for max age
            if (block.GetBlockTime() > (int64_t)tx->nTime + MAX_TX_AGE)
                return state.DoS(50, false, REJECT_INVALID, "bad-tx-time", false, strprintf("%s : transaction timestamp later than max tx age", __func__));
        }
    }
    ...
    if (!isForkBlock(nHeight) && !CheckMinTxOut(block, fV7Enabled))
...
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS=true)
{
    ...
    // mfcoin: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    if (isForkBlock(nHeight)) {
        ...
        if (!utxo_file_path.empty()) {
            ...
            if (!if_utxo.is_open()) {
                ...
                // amount, script, timestamp
                vector<tuple<uint64_t, CScript, uint32_t> > txFromFile;
                ...
                while (if_utxo && recs < forkCBPerBlock) {
                    ...
                    txFromFile.push_back(make_tuple(amount, script, timestamp));
                ...
                if (txFromFile.size() + 1 != block.vtx.size() ||
                    recs + 1 != block.vtx.size()){
                ...
                auto it1 = txFromFile.begin();
                auto it2 = block.vtx.begin() + 1;
                for (; it1 != txFromFile.end() && it2 != block.vtx.end(); ++it1, ++it2) {
                                tuple<uint64_t, CScript, uint32_t>& rec = *it1;
                                const CTransaction& tx = *it2->get();

                                if (get<0>(rec) != tx.vout[0].nValue ||
                                    get<1>(rec) != tx.vout[0].scriptPubKey ||
                                    get<2>(rec) != tx.nTime)
                                {
                                    LogPrintf("AcceptBlock(): FORK Block - Error: Transaction (%d) mismatch\n", txid);
                                    LogPrintf("AcceptBlock(): Transaction: Amount: %d; scriptPubKey: %s; nTime: %d\n", tx.vout[0].nValue, tx.vout[0].scriptPubKey.data(), tx.nTime);
                                    LogPrintf("AcceptBlock(): File Record: Amount: %d; scriptPubKey: %s; nTime: %d \n", get<0>(rec), get<1>(rec).data(), get<2>(rec));
...
bool AddUtxoTransactions(CBlock& genesis)
{
   auto utxo_files = GetUTXOFiles();
   if (utxo_files.empty())
   {
      LogPrintf("ERROR: AddUtxoTransactions(): Cannot locate UTXO files\n");
      return false;
   }
   uint64_t nTx = 0;
   for (const auto& utxo_file : utxo_files)
   {
      //LogPrintf("File %s\n", utxo_file);
      boost::filesystem::ifstream if_utxo(utxo_file, std::ios::binary | std::ios::in);
      if (!if_utxo.is_open()) {
         LogPrintf("ERROR: AddUtxoTransactions(): Cannot open UTXO file - %s\n", utxo_file.string());
         return false;
      }
      ...
      while (if_utxo) {
         ...
         if (!if_utxo.read(coin, 8)) {
            if(!if_utxo.eof())
               LogPrintf("ERROR: AddUtxoTransactions(): UTXO file corrupted? - No more data (Amount)\n");
            break;
         }

         char pubkeysize[8] = {};
         if (!if_utxo.read(pubkeysize, 8)) {
            LogPrintf("ERROR: AddUtxoTransactions(): UTXO file corrupted? - Not more data (PubKeyScript size)\n");
            break;
         }

         int pbsize = bytes2uint64(pubkeysize);
         if (pbsize == 0) {
            LogPrintf("ERROR: AddUtxoTransactions(): UTXO file corrupted? -  PubKeyScript size = 0\n");
            //but proceed
         }

         std::unique_ptr<char[]> pubKeyScript(new char[pbsize]);
         if (!if_utxo.read(&pubKeyScript[0], pbsize)) {
            LogPrintf("ERROR: AddUtxoTransactions(): UTXO file corrupted? Not more data (PubKeyScript)\n");
            break;
         }
         ...
         if(nTx == 0)
            txNew.vin[0].scriptSig = CScript() << 0 << CScriptNum(nTx) << ToByteVector(GetRandHash()) << OP_0;
         else
            txNew.vin[0].scriptSig = CScript() << 0 << CScriptNum(nTx) << OP_0;

         genesis.vtx.push_back(MakeTransactionRef(txNew));
         ++nTx;

         if (!if_utxo.read(&term, 1) || term != '\n') {
            LogPrintf("ERROR: AddUtxoTransactions(): invalid record separator\n");
            break;
         }
      }
   }
   LogPrintf("%d utxos imported\n", nTx);
   return true;
}
...
bool InitBlockIndex(const CChainParams& chainparams)
{
    ...
    if (chainActive.Genesis() != NULL)
        //return true;
        // mfcoin: we want to execute checkpoint key initialization in both cases of this "if"
    ...
    if (!fReindex) {
        try {
            ...
            // mfcoin: calculate pindex->nFlags for genesis block before doing FlushStateToDisk()
...
bool GetMfc7POSReward(const CTransaction& tx, const CCoinsViewCache &view, CAmount &nReward)
{
    nReward = 0;
    ...
    for (const auto& txin : tx.vin)
    {
        ...
        if (pblocktree->ReadTxIndex(prevout.hash, postx))
        {
            ...
            if (txPrev->nTime + Params().GetConsensus().nStakeMinAge > tx.nTime)
    ...
    // Apply 10% APY and round to DP-unit
    nReward = ((bnSatYr * 10 / 100).GetLow64() / TX_DP_AMOUNT) * TX_DP_AMOUNT;

    if (fDebug && GetBoolArg("-printcreation", false))
        LogPrintf("GetMfc7POSReward(): create=%s nCoinAge=%s\n", FormatMoney(nReward), bnSatYr.ToString());
...
bool CheckMinTxOut(const CBlock& block, bool fV7Enabled)
{
    ...
    if (fV7Enabled)
    {
        ...
        {
            // mfcoin: enforce minimum output amount if not witness commitment
